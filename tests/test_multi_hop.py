"""RAGAS quality test for multi-hop (graph-expanded) RAG answers."""
...
def test_multi_hop_quality(multi_hop_rows, ragas_llm, ragas_embeddings, results_dir):
    ...
    @experiment()
    async def run_multi_hop(row: dict) -> dict:
        async with semaphore:
            result = answer_question(row["question"], use_graph=True)
    ...
    dataset = Dataset(name="multi_hop", backend=InMemoryBackend(), data=multi_hop_rows)
    experiment_result = asyncio.run(
        run_multi_hop.arun(
            dataset,
            name="multi_hop_results",
